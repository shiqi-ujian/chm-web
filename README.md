...
- [x] **M7b：注册/账号体系补强（2026-08-21）**：登录支持邮箱/用户名；注册人机校验（算术题 + `NO_CAPTCHA` 测试开关）；重发验证邮件 60s 冷却；修改绑定邮箱（改后重验）；自助注销账号（软删 + 清理会话/用量/文档）；「我的文档」新增账号设置入口；注册表单体验（确认密码 / 人机验证加载兜底 / 注册成功引导去登录）；`npm test` 全绿。
