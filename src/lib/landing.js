...
/* 窄屏文档行：图标/标题列自动换行，操作按钮整行排布，避免按钮挤压 */
@media(max-width:640px){.row-card{gap:8px;padding:14px}.row-card>span:last-child{flex:1 1 calc(100% - 44px);word-break:break-word}}
...
/* 文档标题/作者可能很长：窄屏下禁止撑破卡片，标题最多两行 */
.doc-card .doc-title{white-space:normal;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;word-break:break-word}
...
<div class="header">
  ...
  <nav>
    ...
    <a class="fb-link" href="https://map.chmweb.cn/" target="_blank" rel="noopener" title="战斗地图 · 六角格沙盒 · 通用跑团地图工具">🎲 地图工具</a>
  </nav>
...
<aside class="nav-drawer" id="navDrawer" aria-label="移动端导航">
  ...
  <a href="https://map.chmweb.cn/" target="_blank" rel="noopener">🎲 跑团地图工具</a>
...
<div class="modal" id="authModal">
  <div class="box">
    ...
    <div id="authFieldsLogin">
      <label class="field">用户名或邮箱</label><input class="in" id="au" autocomplete="username" placeholder="用户名或注册邮箱">
    ...
    <div id="authFieldsReg" style="display:none">
      ...
      <label class="field">密码</label><input class="in" id="ap2" type="password" autocomplete="new-password" placeholder="至少 6 位">
      <label class="field">确认密码</label><input class="in" id="apc" type="password" autocomplete="new-password" placeholder="再次输入密码">
      <div class="auth-challenge" style="display:flex;align-items:center;gap:8px;margin-top:10px">
        <span id="authChallengeQ" style="font-size:13.5px;color:var(--ink)"></span>
        <input class="in" id="authChallengeA" style="flex:1;min-width:120px" autocomplete="off" placeholder="答案" inputmode="numeric">
        <button type="button" class="btn ghost sm" id="authChallengeRetry" style="display:none">重试</button>
      </div>
      <div class="err" id="authChallengeErr" style="display:none"></div>
...
var authChallengeToken='',authChallengeAnswer=null,authChallengeFails=0;
  function setChallengeErr(msg,showRetry){try{var ce=document.getElementById('authChallengeErr'),cr=document.getElementById('authChallengeRetry');if(ce){ce.style.display=msg?'':'none';ce.textContent=msg||'';}if(cr)cr.style.display=showRetry?'':'none';}catch(e){}}
  function loadAuthChallenge(){try{var cq=document.getElementById('authChallengeQ');if(!cq)return;if(authChallengeAnswer!=null)return;if(!window.fetch)return;
    setChallengeErr('',false);
    fetch('/api/auth-challenge').then(function(r){return r.json();}).then(function(j){
      if(!j||!j.token){authChallengeFails++;if(authChallengeFails<3){setChallengeErr('人机验证加载失败，请点击重试',true);}else{setChallengeErr('人机验证暂时不可用，可直接提交（失败会提示）',false);}return;}
      authChallengeFails=0;authChallengeToken=j.token;cq.textContent='人机验证：'+j.question;authChallengeAnswer=null;document.getElementById('authChallengeA').value='';setChallengeErr('',false);
    }).catch(function(){authChallengeFails++;if(authChallengeFails<3){setChallengeErr('人机验证加载失败，请点击重试',true);}else{setChallengeErr('人机验证暂时不可用，可直接提交（失败会提示）',false);}});}catch(e){}}
  ...
  var bodyObj;
  if(mode==='register'){
    var ru=document.getElementById('au2').value.trim(),re=document.getElementById('ae').value.trim(),rp=document.getElementById('ap2').value,rc=document.getElementById('apc')?document.getElementById('apc').value:'',acc=document.getElementById('acceptTerms');
    if(!ru||!re||!rp){er.textContent='请填写用户名、邮箱和密码';return;}
    if(rp!==rc){er.textContent='两次输入的密码不一致';return;}
    if(!acc||!acc.checked){er.textContent='请阅读并同意用户协议与隐私政策';return;}
    var chA=document.getElementById('authChallengeA');
    if(chA&&!chA.value.trim()){er.textContent='请完成人机验证（算术题）';return;}
    bodyObj={username:ru,email:re,password:rp,acceptTerms:true,challengeToken:authChallengeToken,challengeAnswer:chA?chA.value.trim():''};
  }else{
  ...
  fetch('/api/'+mode,{method:'POST',headers:Object.assign({'Content-Type':'application/json'},userHeaders()),body:JSON.stringify(bodyObj)})
    .then(function(r){return r.json().then(function(j){return {st:r.status,j:j};});}).then(function(x){
      ...
      else if(x.st===200&&mode==='register'){
        // 注册成功但无 token（服务端返回 {ok:true,username,emailVerified:false}，需邮箱验证后登录）。
        // 原逻辑要求 x.j.token 才走成功分支 → 注册成功被误显示为「失败」，重试报「用户名已存在」。
        er.style.color='#16a34a';er.innerHTML='注册成功，验证邮件已发送，请查收邮箱完成验证后登录。 <a href="#" id="regToLogin" style="color:var(--acc);margin-left:6px">去登录</a>';
      }
...
function goLoginAfterReg(){try{var ru=document.getElementById('au2').value.trim();showLogin();document.getElementById('au').value=ru;document.getElementById('ap').value='';document.getElementById('authErr').textContent='注册成功，请用刚才的用户名/邮箱登录';document.getElementById('authErr').style.color='#16a34a';loadAuthChallenge();}catch(e){}}
  document.addEventListener('click',function(e){if(e.target&&e.target.id==='regToLogin'){e.preventDefault();goLoginAfterReg();}});
...
<div class="foot">CHM 网页 · 免费 · 非营利 · 数据仅用于转换与展示<br>
  <a href="terms.html">用户协议</a> · <a href="privacy.html">隐私政策</a> · <a href="disclaimer.html">免责声明</a> · <a href="report.html">侵权举报</a> · <a href="https://docs.qq.com/form/page/DWkh0VFdOb2tFVFJV" target="_blank" rel="noopener">📝 问题反馈</a> · <a href="https://map.chmweb.cn/" target="_blank" rel="noopener">🎲 跑团地图工具</a><br>
...
const WELCOME = page('CHM 网页 · 免费在线阅读工具', false, `
  ...
  function apiFn(q){fetch('/api/search?q='+encodeURIComponent(q)+'&limit=20').then(function(r){return r.json();}).then(function(j){
    var rows=(j&&j.hits||[]).map(function(h){return {grp:(h.docNs||h.doc)||'',href:h.href||('d/'+(h.doc||'')+'/'),t:(h.title||h.snippet||h.doc||'').slice(0,60),p:h.snippet||''};});render(rows);}).catch(function(){local(q);});}
...
const MINE = page('我的文档 · CHM 网页', true, `
  ...
  <div style="max-width:820px;margin:28px auto 0">
    ...
    <div class="card" style="padding:14px 20px;margin-bottom:16px;display:none" id="privateSearchCard">
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
        ...
        <input class="in" id="privateSearchInput" placeholder="输入关键词…" style="flex:1;min-width:200px" autocomplete="off">
  ...
  <div class="card" id="pwCard" style="max-width:820px;margin:28px auto 0;display:none">
    ...
    <h2 style="font-size:17px;margin:22px 0 4px">修改绑定邮箱</h2>
    <div class="sub">改后需重新验证新邮箱，未验证前不能登录。</div>
    <div style="display:flex;flex-direction:column;gap:10px;max-width:420px;margin-top:14px">
      <input class="in" id="ceEmail" type="email" placeholder="新邮箱" autocomplete="email">
      <div class="err" id="ceErr"></div>
      <div style="display:flex;align-items:center;gap:10px"><button class="btn ghost" id="ceGo" type="button">发送验证邮件到新邮箱</button></div>
    </div>
    <h2 style="font-size:17px;margin:22px 0 4px;color:var(--err)">注销账号</h2>
    <div class="sub">注销后账号不可登录，你上传的文档会一并删除。</div>
    <div style="display:flex;flex-direction:column;gap:10px;max-width:420px;margin-top:14px">
      <div class="err" id="daErr"></div>
      <div style="display:flex;align-items:center;gap:10px"><button class="btn danger" id="daGo" type="button">删除我的账号与全部文档</button></div>
    </div>
  </div>`, `
  ...
  function render(arr){arr=arr||[];
    ...
    box.innerHTML=filterHtml + arr.map(function(d){
      ...
      return '<div class="card row-card" style="flex-wrap:wrap"><label style="display:flex;align-items:flex-start;cursor:pointer;padding:4px 0 4px 4px"><input type="checkbox" class="row-check" data-id="'+escS2(d.id)+'"'+checked+'></label>'+
      '<span style="font-size:24px;padding-top:4px">📘</span>'+
      '<span style="flex:1;min-width:calc(100% - 120px);word-break:break-word"><b style="display:block">'+escS2(d.name||d.id)+'</b>'+
      ...
      (d.tags||[]).map(function(t){return '<span class="tag" data-tag="'+escS2(t)+'" style="cursor:pointer;margin-top:2px">#'+escS2(t)+'</span>';}).join('')+
      '</span>'+
      '<div style="flex:100%;display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">'+
        '<a href="'+escS2(d.href||(d.visibility==='private'?'p/':'d/')+d.id+'/')+'" class="btn ghost sm">打开</a>'+
        '<button class="btn ghost sm" data-edit="'+escS2(d.id)+'" data-name="'+escS2(d.name||d.id)+'" data-author="'+escS2(d.author||'')+'" data-tags="'+escS2((d.tags||[]).join(','))+'">编辑</button>'+
        '<button class="btn ghost sm" data-vis="'+escS2(d.id)+'">'+(d.visibility==='private'?'设为公开':'设为私密')+'</button>'+
        '<button class="btn ghost sm" data-share="'+escS2(d.id)+'">分享</button>'+
        '<button class="btn danger sm" data-del="'+escS2(d.id)+'">删除</button></div></div>';}).join('');
  ...
  // ---- 修改绑定邮箱 / 注销账号（仅登录后可见，随账号区一起显示） ----
  var ceGo=document.getElementById('ceGo'),daGo=document.getElementById('daGo');
  function changeEmail(){var e=document.getElementById('ceEmail'),er=document.getElementById('ceErr');
    var v=(e&&e.value||'').trim();if(!v){er.style.color='';er.textContent='请输入新邮箱';return;}
    if(!/@[^@]+@[^@]+\./.test(v)){er.style.color='';er.textContent='邮箱格式不正确';return;}
    er.textContent='';if(ceGo)ceGo.disabled=true;
    fetch('/api/change-email',{method:'POST',headers:Object.assign({'Content-Type':'application/json'},userHeaders()),body:JSON.stringify({email:v})})
      .then(function(r){return r.json().then(function(j){return {st:r.status,j:j};});}).then(function(x){
        if(ceGo)ceGo.disabled=false;
        if(x.st===200){er.style.color='#16a34a';er.textContent='已发送验证邮件到新邮箱，请查收并完成验证';if(e)e.value='';}
        else{er.style.color='';er.textContent=(x.j&&x.j.error)||'修改失败';}})
      .catch(function(e){if(ceGo)ceGo.disabled=false;er.style.color='';er.textContent='网络错误：'+e.message;});}
  if(ceGo)ceGo.addEventListener('click',changeEmail);
  function deleteAccount(){var er=document.getElementById('daErr');
    if(!window.confirm('确认注销账号？你的账号与上传的全部文档都会被删除，不可恢复。'))return;
    er.textContent='';if(daGo)daGo.disabled=true;
    fetch('/api/delete-account',{method:'POST',headers:userHeaders()})
      .then(function(r){return r.json().then(function(j){return {st:r.status,j:j};});}).then(function(x){
        if(x.st===200){USER_TOKEN='';currentUser=null;try{localStorage.removeItem('chm_user');}catch(e){}renderAuth();afterMe();window.location.href='index.html';}
        else{if(daGo)daGo.disabled=false;er.style.color='';er.textContent=(x.j&&x.j.error)||'注销失败';}})
      .catch(function(e){if(daGo)daGo.disabled=false;er.style.color='';er.textContent='网络错误：'+e.message;});}
  if(daGo)daGo.addEventListener('click',deleteAccount);
