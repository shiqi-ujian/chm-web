...
var authChallengeToken='',authChallengeAnswer=null;
  function loadAuthChallenge(){try{var cq=document.getElementById('authChallengeQ');if(!cq)return;if(authChallengeAnswer!=null)return;if(!window.fetch)return;
    fetch('/api/auth-challenge').then(function(r){return r.json();}).then(function(j){if(!j||!j.token)return;authChallengeToken=j.token;cq.textContent='人机验证：'+j.question;authChallengeAnswer=null;document.getElementById('authChallengeA').value='';})
  ...
  fetch('/api/'+mode,{method:'POST',headers:Object.assign({'Content-Type':'application/json'},userHeaders()),body:JSON.stringify(bodyObj)})
    .then(function(r){return r.json().then(function(j){return {st:r.status,j:j};});}).then(function(x){
      if(x.st===200&&x.j.token){USER_TOKEN=x.j.token;try{localStorage.setItem('chm_user',USER_TOKEN);}catch(e){}
        ...
        er.style.color='#16a34a';er.textContent='注册成功，验证邮件已发送，请查收邮箱完成验证后登录';
      }
