...
<style>
  ...
  *{box-sizing:border-box}
  ...
  @media(max-width:560px){.steps{grid-template-columns:1fr}.top h1{font-size:24px}}
</style></head><body>
...
(function(){
  ...
  (function initDocs(){ renderDocs(null); refreshDocs(); })();

  drop.addEventListener('click',function(){ file.click(); });
  drop.addEventListener('dragover',function(e){ e.preventDefault(); drop.classList.add('over'); });
  drop.addEventListener('dragleave',function(){ drop.classList.remove('over'); });
  drop.addEventListener('drop',function(e){ e.preventDefault(); drop.classList.remove('over'); if(e.dataTransfer.files&&e.dataTransfer.files[0]) select(e.dataTransfer.files[0]); });
  file.addEventListener('change',function(){ if(file.files[0]) select(file.files[0]); });

  function select(f){
    if(!/\.chm$/i.test(f.name)){ msg.innerHTML='<span class="err">请选择 .chm 文件</span>'; return; }
    chosen=f; pick.textContent='已选择：'+f.name+' · '+(f.size/1024).toFixed(1)+' KB'; pick.classList.add('show');
    msg.innerHTML='';
  }

  // 真实上传（有后端时）
  go.addEventListener('click',function(){
    if(!chosen){ msg.innerHTML='<span class=err>请先选择一个 .chm 文件</span>'; return; }
    if(!window.fetch || !window.FormData){ msg.innerHTML='<span class=err>当前环境不支持上传。</span>'; return; }
    var fd=new FormData(); fd.append('file', chosen);
    go.disabled=true; go.textContent='转换中…';
    msg.innerHTML='<span class=ok>正在上传并转换，大约几秒…</span>';
    fetch('/api/upload',{method:'POST',body:fd}).then(function(res){
      ...
      if(x.st===200 && x.j && x.j.url){
        msg.innerHTML='<span class=ok>转换成功！即将打开新文档…</span>';
        renderDocs(); refreshDocs();
        setTimeout(function(){ location.href=x.j.url; }, 900);
      } else {
        msg.innerHTML='<span class=err>'+esc((x.j&&x.j.error)||'转换失败')+'</span>';
      }
    }).catch(function(e){
      go.disabled=false; go.textContent='转换文档';
      msg.innerHTML='<span class=err>上传失败：'+esc(e&&e.message||e)+'</span>';
    });
  });
})();
</script>
