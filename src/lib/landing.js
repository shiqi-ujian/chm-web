...
(function(){
  ...
  (function renderDocs(){
    ...
    if(!DOCS.length){ box.innerHTML='<div class="doc-row"><span style="color:var(--mut)">还没有文档，上传第一个吧。</span></div>'; return; }
    box.innerHTML=DOCS.map(function(d){
      return '<div class="doc-row"><div><div class="name">'+esc(d.name)+'</div><span class="tag">公开</span></div>'+
             '<a href="'+escH(d.href)+'">打开 ›</a></div>';
    }).join('');
  })();
  ...
  go.addEventListener('click',function(){
    ...
    msg.innerHTML='<span class=ok>服务器正在准备，上传功能即将开放。你可以先把示例文档分享给朋友看效果。</span>';
  });
})();
</script>
</body></html>`;
