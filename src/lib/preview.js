...
.side-foot{padding:8px 14px;border-top:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;gap:8px}
#theme-btn{flex:0 0 auto;border:1px solid var(--border);background:var(--bg);border-radius:8px;padding:5px 12px;cursor:pointer;font-size:14px;color:var(--text)}
...
.tb-btn{flex:0 0 auto;border:1px solid var(--border);background:var(--bg);border-radius:8px;padding:5px 10px;cursor:pointer;font-size:14px;color:var(--text);line-height:1}
...
@media(max-width:680px){
  ...
  /* 移动端顶栏只保留核心导航按钮：页内搜索由底部操作条承担，避免顶栏过挤 */
  #page-search-btn{display:none}
  ...
  #mobile-bar .mb-btn{flex:1;text-align:center;border:0;background:transparent;color:var(--text);font-size:13px;padding:4px 0;line-height:1.2;cursor:pointer;-webkit-tap-highlight-color:transparent}
  #mobile-bar .mb-btn:hover{color:var(--accent)}
  #mobile-bar .mb-btn:active{background:var(--hover)}
  #mobile-bar .mb-btn b{display:block;font-size:16px;font-weight:600}
  #mobile-bar .mb-btn:disabled b{opacity:.4}
  #main{padding-bottom:env(safe-area-inset-bottom)}
  #app{min-height:0}
  /* 侧栏抽屉头部固定，目录区可独立滚动 */
  #sidebar .side-head{flex:0 0 auto}
  #toc{flex:1 1 auto;overflow-y:auto;-webkit-overflow-scrolling:touch}
  #sidebar .side-foot{flex:0 0 auto}
  /* 顶栏按钮在超窄屏下更紧凑 */
...
/* iframe 加载完成后补一次阅读样式，确保字号/进度/排版在真实正文页上生效 */
if (frame) frame.addEventListener('load', function(){
  try { applyFontSize(fontSizeStep); } catch(e){}
  try { applyReadingStyle(); } catch(e){}
...
function openDrawer(){
  ...
  try {
    ...
    var rows = tocEl.querySelectorAll('.toc-row');
    for (var i=0;i<rows.length;i++){
      if (rows[i].parentElement.dataset.id === String(cur.i)){
...
function currentUrl(){
  ...
  try {
    ...
    if (w && w.location && w.location.href && !/^about:blank$/i.test(w.location.href)) {
      src = w.location.href;
    }
  } catch(e){}
...
var psPageBtn = document.getElementById('page-search-btn');
if (psPageBtn) psPageBtn.addEventListener('click', function(){
...
var psPrev = document.getElementById('ps-prev');
var psNext = document.getElementById('ps-next');
if (psPrev) psPrev.addEventListener('click', function(){ if (psTotal) { psCur = (psCur - 1 + psTotal) % psTotal; scrollToMark(psCur, true); } });
if (psNext) psNext.addEventListener('click', function(){ if (psTotal) { psCur = (psCur + 1) % psTotal; scrollToMark(psCur, true); } });
var psClose = document.getElementById('ps-close');
if (psClose) psClose.addEventListener('click', function(){ psBar.classList.remove('on'); clearHl(fdoc()); });
...
/* Ctrl+K / Cmd+K：聚焦站内/全库搜索框（桌面快捷键，移动端键盘快捷键不常用但无害） */
document.addEventListener('keydown', function(e){
  if ((e.ctrlKey || e.metaKey) && (e.key === 'k' || e.key === 'K')){
    e.preventDefault();
    if (q){ q.focus(); q.select(); setTimeout(function(){ q.focus(); }, 0); }
    else { var el = document.getElementById('q'); if (el){ el.focus(); el.select(); } }
  }
});
if (q){
