...
// 验证每用户文档数上限、全局存储上限、限流 429。用真实上传验证不会污染仓库。
...
function upload(token, filename) {
  return new Promise((resolve) => {
    ...
    const r = http.request({ host: 'localhost', port: '18084', path: '/api/upload', method: 'POST', headers: h }, (x) => {
      let b = []; x.on('data', (c) => b.push(c)); x.on('end', () => resolve({ st: x.statusCode, body: JSON.parse(Buffer.concat(b).toString() || '{}') }));
    });
    r.on('error', (e) => resolve({ st: 0, body: { err: e.message } }));
    r.write(body); r.end();
...
async function main() {
  ...
  // 限流：50f 并发 upload 请求（已超额度，但限流层先行），应出现 429
  let nonAuth429 = 0;
