...
| M7 | 登录加固 + 移动端 + 合规 | ✅ 已完成：注册需邮箱+同意条款、邮箱验证、忘记/重置密码、失败锁定、CSRF；移动端优化（汉堡菜单、上传权利勾选、阅读壳适配）；新增 `terms/privacy/disclaimer/report.html`，举报入库 + 管理后台 `admin.html`（状态流转/下架）；测试 `test-auth-v2` 并入 `npm test`，15 项全绿。后续补强：未验证邮箱禁止登录 + 验证链接自动登录；登录支持邮箱；注册人机校验；重发冷却；改邮箱；自助注销（2026-08-21） |
...
| M9·私密搜索/分享/体验/性能 | 私密文档全文搜索 + 分享链接生命周期 + 阅读壳与大文档性能 | ✅ 已完成（2026-08-20）：「我的文档」搜索私密文档、分享链接有效期/撤销；私有阅读壳 `/p/` 前缀 + URL 归一化；移动端体验第二批；大文档性能（hhc 线性化/惰性树/rename 迁移/异步重建/gzip+缓存）；邮箱验证强制与 SMTP 修复；私有导出 ACL 安全修复 |
...
### 15. 私有文档阅读壳 `/p/` 前缀与移动端设置失效修复（2026-08-20）
- 现象：私有文档正文 iframe / TOC / 首页 URL 曾缺 `/p/` 前缀或前缀重复，导致转私后 404；移动端字号/排版/进度按钮不生效。
- 修复：
  - 阅读壳 TOC/首页/正文链接统一使用绝对 `/p/<id>/` 前缀；
  - 公开转私有后立即重建私有阅读壳（避免漏重建）；
  - 服务启动时自动重建存量私有文档阅读壳 + 启动提示重启（老壳自愈）；
  - 字号/排版/进度改为 iframe `load` 后再应用，移动端按钮恢复可用。
- 后续（当天晚间 commit `dcf449b`）补充 URL 归一化：`normPath` 统一私有壳绝对路径与 TOC key，目录高亮/上一页/下一页/进度恢复；转公开后也重建阅读壳（清除残留 `/p/` 前缀）。
- 自测：`npm test` 全绿；相关提交已补齐 CHANGELOG。

### 19. 注册表单体验优化（2026-08-21）
- 注册弹窗新增“确认密码”输入，前端校验两次密码一致后再提交。
- 人机验证题加载失败时显示内嵌错误 + “重试”按钮；连续失败 3 次后允许不带题直接提交（服务端仍会返回 400，不硬卡注册）。
- 注册成功改为绿色提示 + “去登录”链接，点击后切到登录视图并预填用户名。
- `npm test` 全绿（含 `test-site` 的页面脚本语法检查）。

### 18. 注册/账号体系补强（2026-08-21）
- **登录支持邮箱/用户名**：`login` 对形如邮箱的输入按 `lower(email)` 查用户，返回规范用户名；忘记用户名也能登录。
- **注册人机校验**：`GET /api/auth-challenge` 发一次性算术题（5 分钟有效），`POST /api/register` 提交 `challengeToken` + `challengeAnswer`；`NO_CAPTCHA=1` 可跳过（测试/本地）。
- **重发验证邮件冷却**：`requestEmailVerification` 检查 `verification_expires` 未到期的已有验证码，1 分钟内禁止重发。
- **修改邮箱**：登录态 `POST /api/change-email`，新邮箱唯一性校验；改后 `email_verified=0` 并重发验证邮件。
- **自助注销**：登录态 `POST /api/delete-account`；事务内删会话/用量、解除 meta owner、用户软删（`deleted_at` + 清空邮箱），再由 server 回调删除文档实体并重建站点；前端「我的文档 → 账号设置」提供入口。
- **自测**：`test-auth-v2` 增补邮箱登录/改邮箱/注销断言；`scripts/launch-check.js` 增加生产 `NO_CAPTCHA` 检查；`npm test` 全绿。

### 17. 昨晚批量性能/邮件/验证安全收尾（2026-08-20 晚）
- **性能**：
  - `parseHhc` 从 O(n²) 改为线性正则扫描，7000 节点 20s → 17ms；
  - 阅读壳目录树惰性渲染（只渲染顶层，展开时 `ensureKids`），7000 节点目录初始 DOM 从 7000+ 降到 17；
  - 转私有/公开改 `rename` 迁移 + 壳重建 `skipIndexes` + `queueRebuildSite()` 异步合并重建，大文档不再卡死；
  - 静态文本资源 gzip + `Cache-Control`（API `no-store`、`sw.js` `no-cache`），弱网刷新大幅提速。
- **邮箱验证闭环**：注册绑定真实邮箱后未验证禁止登录（legacy 无邮箱豁免）；注册成功前端不再误报失败；验证链接 GET 自动登录跳首页；`test-auth` 增补 403/验证后登录断言。
- **SMTP 修复**：`send` 未定义、DATA 缺 RFC5322 From、成功未 resolve（20s 超时误报）三连坑修复；新增 `SMTP_FROM_NAME` 发件显示名（RFC2047）；QQ SMTP 实测发送成功。
- **安全**：私有文档导出 ACL 修复——`EXPORT_TOKEN` 未配置时匿名导出私有文档 403，防绕过 `/p/` ACL。
- **用量校准**：`reconcileUsage` 启动时按 meta + 磁盘重算 `user_usage`，修复已用容量失实。
- CHANGELOG 与 README/HANDOFF/PLAN 已随文补记到位。

### 16. 移动端体验优化第二批（2026-08-20）
- 阅读壳「返回首页」改为回到站点根（`/d/<id>/` 或 `/p/<id>/` → 站点根），兼容子路径与离线导出（commit `43a064e`）。
- 打开移动端目录抽屉时自动滚动到当前章节；窄屏顶栏隐藏重复的页内搜索按钮（底部操作条已承担）。
- 新增 `Ctrl+K` / `Cmd+K` 聚焦全库搜索；页内搜索按钮增加空值保护，避免桌面/移动端结构差异时报错。
- 首页公开文档卡片、浏览页文档行整张可点击打开；浏览页标签筛选仍独立触发不误跳转。
- `test-site.js` 增加阅读壳移动端结构断言；`npm test` 全绿。
...
- [x] 私密文档全文搜索 + 分享链接有效期/撤销（M9）——「我的文档」可搜自己的私密文档；分享链接可设有效期、查看、撤销；`test-share-mgmt.js` 已接入
- [x] 私有文档阅读壳 `/p/` 前缀 + 移动端字号/排版/进度修复（2026-08-20）——转私后不再 404，按钮在移动端可生效；已补 CHANGELOG 与 PROGRESS
- [x] 移动端体验优化第二批（2026-08-20）——返回列表/整卡跳转/抽屉滚动/Ctrl+K/顶栏精简；已补 CHANGELOG 与 PROGRESS
- [x] 大文档性能优化与邮件/验证/安全收尾（2026-08-20 晚）——hhc 线性化（20s→17ms）、目录惰性渲染、rename 迁移、异步重建、gzip+缓存；登录强制邮箱验证、SMTP 修复与 `SMTP_FROM_NAME`、私有导出 ACL 403；已补 CHANGELOG 与 PROGRESS
- [x] 注册/账号体系补强（2026-08-21）——登录支持邮箱/用户名；注册人机校验（算术 challenge）；重发验证邮件 60s 冷却；修改绑定邮箱（改后重验）；自助注销账号（软删 + 清理会话/用量/文档）；注册表单体验（确认密码 / 人机验证兜底 / 注册成功去登录）；`test-auth-v2` 增补用例，`npm test` 全绿
- [ ] 正文中文翻译（需外部服务/预算评估）
- [ ] autosync 在多台设备上的部署说明
- [ ] PROGRESS/README/HANDOFF 按最新线上与功能状态持续归档（有更新随提交补）
- [x] **核对阿里云自动部署链路（2026-08-21 已确认）**：服务器侧已配置自动拉取部署，`push main → 线上自动更新` 实际生效（仓库内只有测试 workflow，部署由服务器侧 webhook / cron / systemd timer 负责）
